...
public class StartChallenge {
	...
	/**
	 * Starts the program. args[0] must be the path to the input csv-file.
	 * 
	...
	public static void main(String[] args) throws FileNotFoundException {
		if(args.length < 1) {
			throw new IllegalArgumentException("Expected args[0] to be path to input csv-file, but nothing was given.");
		}
		// Load input file
		...
		// Init workers for multithreading, i.e. every worker should work one thread
		// 1. finding shortest paths on 1/NUM_WORKERS of all permutations
		if (PathFinder.NUM_WORKERS > nodes.size() / 2 + 1) {
			PathFinder.NUM_WORKERS = nodes.size() / 2 + 1;
			System.out.println(
					"Too many cores for too few nodes, reduced number of workers to " + PathFinder.NUM_WORKERS);
		}
		...
		for (int i = 0; i < workers.length; i++) {
			...
			// ... but choose another element for index=0 based on number of workers
			// available.
		...
		// Find global shortest path in all the workers' results
		...
		// Print global shortest path
		System.out.println("\nShortest of all paths found. Length=" + globalShortestPathLength
				+ ", order of waypoints is: " + listToString(globalShortestPath));
	}
